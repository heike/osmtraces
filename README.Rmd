...
```{r}
ames_tracks <- read_osm_traces("ames")
```

Tracks have names - some tracks are split across different files. 
Unnesting and re-nesting brings these tracks together:
...
ames_tracks <- ames_tracks |> filter(track_length > 1)
...
ames_trackpoints |> ggplot(aes(x = lon, y = lat, colour = gps_heading)) +
  geom_path(aes(group=rowid), alpha = 0.7, linewidth=1) +
